    ...
    public class CD_VentaObra
    {
        public int ObtenerCorrelativo()
        {
            ...
            using (SqlConnection oconexion = new SqlConnection(Conexion.cadena))
            {
                ...
                try
                {
                    ...
                    query.AppendLine("select count(*) + 1 from VENTA_OBRA");
        ...
        public bool Registrar(Venta_Obra obj, DataTable DetalleVentaObra, out string Mensaje)
        {
            ...
            try
            {
                using (SqlConnection oconexion = new SqlConnection(Conexion.cadena))
                {
                    SqlCommand cmd = new SqlCommand("usp_RegistrarVentaObra", oconexion);
                    ...
                    cmd.Parameters.AddWithValue("DetalleVentaObra", DetalleVentaObra);
        ...
        public Venta_Obra ObtenerVenta(string numero)
        {
            ...
            using (SqlConnection conexion = new SqlConnection(Conexion.cadena))
            {
                try
                {
                    ...
                    query.AppendLine("select v.IdVentaObra,u.NombreCompleto,");
                    ...
                    query.AppendLine("from VENTA_OBRA v");
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        ...
                        while (dr.Read())
                        {
                            obj = new Venta_Obra()
                            {
                                IdVentaObra = int.Parse(dr["IdVentaObra"].ToString()),
        ...
        public List<Detalle_Venta_Obra> ObtenerDetalleVenta(int idVentaObra)
        {
            List<Detalle_Venta_Obra> oLista = new List<Detalle_Venta_Obra>();
            ...
            using (SqlConnection conexion = new SqlConnection(Conexion.cadena))
            {
                try
                {
                    conexion.Open();
                    StringBuilder query = new StringBuilder();
                    query.AppendLine("select p.Nombre,dv.PrecioVenta,dv.Cantidad,dv.Subtotal from DETALLE_VENTA_OBRA dv");
                    query.AppendLine("inner join PRODUCTO p on p.IdProducto = dv.IdProducto");
                    query.AppendLine("where dv.IdVenta_Obra = @idventaobra;");

                    SqlCommand cmd = new SqlCommand(query.ToString(), conexion);
                    cmd.Parameters.AddWithValue("@idventaobra", idVentaObra);
                    cmd.CommandType = System.Data.CommandType.Text;
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            oLista.Add(new Detalle_Venta_Obra()
                            {
                ...
                {
                    oLista = new List<Detalle_Venta_Obra>();
                }
            }
            return oLista;
        }
